...
class DeleteNft
{
    async removeNft(req,res)
    {
        try
        {
           ...
           if (artworks.Items.hasOwnProperty(metadata)){
            await IPFS.prototype.deleteImage(artworks.Items.metadata) 
           }
           if(artworks.Items.hasOwnProperty(placeholder_image)){
            S3.prototype.deleteImage(artworks.Items.placeholder_image)
           }
           await IPFS.prototype.deleteImage(artworks.Items.cid)
    ...
    async deletePlaceholderImage(req,res)
    {
        try
        {
           ...
           if (artworks.Items.hasOwnProperty(placeholder_image) && artworks.Items.placeholder_image!=""){
            S3.prototype.deleteImage(artworks.Items.placeholder_image)
            const updatedParams={
                TableName:'puffles',
                Key:{PK:`ADR${req.user.address}`,SK:`ART${req.body.artwork_id}`},
                UpdateExpression:"set #placeholder_image=:placeholder_image,#placeholder_fileurl=:filenames",
                ExpressionAttributeNames:{"#placeholder_image":"placeholder_image","#placeholder_fileurl":"placeholder_fileurl"},
                ExpressionAttributeValues:{":placeholder_image":"",":filenames":""}
            }
            ...
            res.status(200).json({status:false,message:"Artwork does not contain Placeholder Image deleted successfully"});
        }
            }
        }
        catch(err)
